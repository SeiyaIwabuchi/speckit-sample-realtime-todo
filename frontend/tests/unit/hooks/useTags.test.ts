...
describe('useTags', () => {
  ...
  it('should subscribe and set tags', () => {
    (tagService.subscribeTags as jest.Mock).mockImplementation((cb) => {
      cb(mockTags);
      return jest.fn();
    });
  ...
  it('should call createTag and handle error', async () => {
    (tagService.createTag as jest.Mock).mockResolvedValueOnce(undefined);
    const { result } = renderHook(() => useTags());
    await act(async () => {
      await result.current.createTag({ name: '新規', color: '#3B82F6', userId: 'user1' });
    });
    expect(tagService.createTag).toHaveBeenCalled();
    (tagService.createTag as jest.Mock).mockRejectedValueOnce(new Error('重複'));
    await act(async () => {
  ...
  it('should call updateTag and handle error', async () => {
    (tagService.updateTag as jest.Mock).mockResolvedValueOnce(undefined);
    const { result } = renderHook(() => useTags());
    await act(async () => {
      await result.current.updateTag('1', { name: '更新' });
    });
    expect(tagService.updateTag).toHaveBeenCalled();
    (tagService.updateTag as jest.Mock).mockRejectedValueOnce(new Error('更新失敗'));
    await act(async () => {
  ...
  it('should call deleteTag and handle error', async () => {
    (tagService.deleteTag as jest.Mock).mockResolvedValueOnce(undefined);
    const { result } = renderHook(() => useTags());
    await act(async () => {
      await result.current.deleteTag('1');
    });
    expect(tagService.deleteTag).toHaveBeenCalled();
    (tagService.deleteTag as jest.Mock).mockRejectedValueOnce(new Error('削除失敗'));
    await act(async () => {
